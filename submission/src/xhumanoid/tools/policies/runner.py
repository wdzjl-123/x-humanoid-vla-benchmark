...
def parse_args():
    ...
    parser.add_argument(
        "--task-publish-delay", type=float, default=5.0,
        help="Seconds to wait after binding the action PUB socket before publishing --task (default: 5)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--prewarm-steps", type=int, default=0,
        help="Run synthetic ACT inference steps before binding ZMQ or publishing a task",
    )
    # ACT 参数
    parser.add_argument(
...
def prewarm_act_policy(policy, args) -> None:
    """Compile CUDA kernels before a formal task is published."""
    steps = int(args.prewarm_steps)
    if steps <= 0:
        return
    if args.policy not in ("act", "act_v1", "multitask_act"):
        print(f"[runner] Prewarm skipped for policy={args.policy}")
        return

    image_height = int(policy.IMG_H)
    image_width = int(policy.IMG_W)
    observation = {
        "puppet": {
            "arm_left_position_raw": {"data": np.zeros(7, dtype=np.float32)},
            "end_effector_left_position_raw": {"data": np.zeros(6, dtype=np.float32)},
            "arm_right_position_raw": {"data": np.zeros(7, dtype=np.float32)},
            "end_effector_right_position_raw": {"data": np.zeros(6, dtype=np.float32)},
            "end_effector_left_pose_raw": {"data": np.zeros(7, dtype=np.float32)},
            "end_effector_right_pose_raw": {"data": np.zeros(7, dtype=np.float32)},
        },
        "camera_observations": {
            "color_images": {"camera_head": np.zeros((image_height, image_width, 3), dtype=np.uint8)},
            "depth_images": {"camera_head": np.zeros((image_height, image_width), dtype=np.uint16)},
        },
    }
    for _ in range(steps):
        action = policy.infer(observation)
        if not np.all(np.isfinite(action)):
            raise RuntimeError("Prewarm produced non-finite action")
    policy.reset()
    print(f"[runner] Prewarm complete: steps={steps}")
...
def run_inference_server():
    ...
    if args.prewarm_steps < 0:
        raise ValueError("prewarm-steps must be non-negative")
    if args.task_publish_delay < 0:
        raise ValueError("task-publish-delay must be non-negative")
    policy = _PolicyCls(args.model_path, args.device, **extra_kwargs)
    prewarm_act_policy(policy, args)
    ...
    try:
        ...
        if args.task:
            if args.task_publish_delay > 0:
                print(
                    "[runner] Waiting "
                    f"{args.task_publish_delay:.1f}s after PUB bind for the simulator subscription"
                )
                time.sleep(args.task_publish_delay)
