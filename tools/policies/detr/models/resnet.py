...
class BasicBlock(nn.Module):
    ...
    def forward(self, x, language_embed=None):
        ...
        # print(f"resent film BasicBlock is used! use_film: {self.use_film}")
        # print(f"BasicBlock language_embed: {language_embed.size()}")
        if self.use_film and language_embed is not None:
            # print(f"resent film BasicBlock language_embed is used!")
...
class Bottleneck(nn.Module):
    ...
    def forward(self, x, language_embed=None):
        ...
        print(f"resent file Bottleneck is used! use_film: {self.use_film}")
        print(f"Bottleneck language_embed: {language_embed.size()}")
        if self.use_film and language_embed is not None:
            print(f"resent film Bottleneck language_embed is used!")
            out = self.film(out, language_embed)
...
#         return super().forward(x, language_embed)


# class FilmedBottleneck(Bottleneck):
#     def forward(self, x, y, z):
#         print(f"FilmedBottleneck forward is called!")
#         # if language_embed is not None:
#             # return filmed_forward(self, x, language_embed)
...
class CustomResNet(ResNet):
    def __init__(self, block, layers, use_film, **kwargs):
        self.use_film = use_film
    ...
    def _make_layer(
        ...
        layers = []
        print(f"CustomResNet _make_layer is called!")
    ...
    def forward(self, x, language_embed=None):
        ...
        if language_embed is not None:
            # language_embed size: torch.Size([16, 768])
            print(f'language_embed size: {language_embed.size()}')

        if isinstance(self.layer1[0], (BasicBlock, Bottleneck)) and self.layer1[0].use_film:
            print(f"self.layer1 is good")
        if isinstance(self.layer2[0], (BasicBlock, Bottleneck)) and self.layer2[0].use_film:
            print(f"self.layer2 is good")
        if isinstance(self.layer3[0], (BasicBlock, Bottleneck)) and self.layer3[0].use_film:
            print(f"self.layer3 is good")
        if isinstance(self.layer4[0], (BasicBlock, Bottleneck)) and self.layer4[0].use_film:
            print(f"self.layer4 is good")
...
def _resnet(arch, block, layers, weights, use_film, **kwargs):
    model = CustomResNet(block, layers, use_film, **kwargs)
    # if use_film:
